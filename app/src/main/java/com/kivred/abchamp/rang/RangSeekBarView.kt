...
@Suppress("unused", "CustomViewStyleable", "Type mismatch")
class RangSeekBarView : View {
    ...
    private var rangPickerBehavior: RangPickerBehavior = RangPickerBehavior.CUBIC
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        attrs?.let {
            ...
            stretchRange = typedArray.getDimensionPixelSize(R.styleable.RubberSeekBar_stretchRange, convertDpToPx(context, 24f).toInt()).toFloat()
            drawableThumbRadius = typedArray.getDimensionPixelSize(R.styleable.RubberSeekBar_defaultThumbRadius, convertDpToPx(context, 16f).toInt()).toFloat()
            normalTrackWidth = typedArray.getDimensionPixelSize(R.styleable.RubberSeekBar_normalTrackWidth, convertDpToPx(context, 2f).toInt()).toFloat()
            highlightTrackWidth = typedArray.getDimensionPixelSize(R.styleable.RubberSeekBar_highlightTrackWidth, convertDpToPx(context, 4f).toInt()).toFloat()
            ...
            rangPickerBehavior = typedArray.getInt(R.styleable.RubberSeekBar_elasticBehavior, 1).run {
                when (this) {
                    ...
                    1 -> RangPickerBehavior.CUBIC
                    2 -> RangPickerBehavior.RIGID
                    else -> RangPickerBehavior.CUBIC
                }
            }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(getDefaultSize(suggestedMinimumWidth, widthMeasureSpec), measureDimension(minimumHeight + paddingTop + paddingBottom, heightMeasureSpec))
    }
    ...
    private fun drawThumb(canvas: Canvas?) {
        if (rangPickerBehavior == RangPickerBehavior.RIGID) {
            thumbY = trackY
        }
    ...
    private fun drawTrack(canvas: Canvas?) {
        ...
        when (rangPickerBehavior) {
            RangPickerBehavior.LINEAR -> drawLinearTrack(canvas)
            RangPickerBehavior.CUBIC -> drawBezierTrack(canvas)
            RangPickerBehavior.RIGID -> drawRigidTrack(canvas)
        }
    }
    ...
    fun getElasticBehavior(): RangPickerBehavior {
        return rangPickerBehavior
    }
     ...
     */
    fun setElasticBehavior(rangPickerBehavior: RangPickerBehavior) {
        this.rangPickerBehavior = rangPickerBehavior
        if (rangPickerBehavior == RangPickerBehavior.RIGID) {
            springAnimation?.cancel()
        }
        invalidate()
    ...
    fun setCurrentValue(value: Long) {
        ...
        if (trackEndX < 0) {
            ...
            if (initialControlXPositionQueue.isNotEmpty()) {
                //Incas this is called multiple times, always use the latest value
